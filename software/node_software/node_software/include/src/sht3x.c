...
uint8_t sht3x_check_crc(uint8_t *data) {
	...
	uint32_t working_data = ( (uint32_t) ( data[0] ^ 0xFF ) << 16 ) | (uint32_t) ( data[1] << 8 ) | (uint32_t) data[2];
	uint8_t is_success;
	uint16_t CRC;
	...
	if (working_data == 0x00000000) { // if remain is 0
		is_success = 0;
	}
	else {
		is_success = 1;
...
uint8_t sht3x_decode_data(uint8_t *data, uint8_t is_only_temperature, float *temperature, float *humidity) {
 ...
 uint8_t sht3x_measurement(uint8_t reliability, float *temperature, float *humidity, uint8_t is_only_temperature) {
	...
	if (is_only_temperature) {
		data_length = 4; // include the address+R byte which will be copied because of in the buffer 
	}
	else {
		data_length = 7; // include the address+R byte which will be copied because of in the buffer
	}
	if (SHT3X_CLOCK_STRETCHING) {
	...
	if (command) {
		uint8_t data[7] = {(SHT3X_ADDRESS<<1), (uint8_t) (command>>8), (uint8_t) command};
		...
		if (i2c_get_status()) {
			if (!SHT3X_CLOCK_STRETCHING) {
				...
				i2c_start_write(data, data_length);
			...
			if (i2c_read_buffer(data, data_length)) { 
				if (sht3x_decode_data(data+1, is_only_temperature, temperature, humidity)) {
					is_success = 1;
				}
				else {
					is_success = 0;
